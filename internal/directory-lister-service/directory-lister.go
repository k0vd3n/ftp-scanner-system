...
func (s *directoryListerService) ProcessDirectory(ctx context.Context, scanMsg *models.DirectoryScanMessage, ftpRepo ftpclient.FtpClientInterface) error {
	...
	// ------------ Запуск листинга директории с ретраями ------------
	...
	// инкремент метрики количества обработанных директорий
	DirectoriesProcessed.Inc()

	if len(dirs) != 0 {
		// ------------ Отправка найденных поддиректорий в топик ------------
		for _, dir := range dirs {
			...
			if err := s.retryKafkaSend(
				...
				s.logger.Info("Directory-lister-service: ProcessDirectory - Поддиректория на листинг отправлена в Kafka",
					...
					zap.String("topic", s.config.DirectoriesToScanTopic),
					zap.String("scan_id", scanMsg.ScanID))
				// инкремент метрики количества отправленных директорий
				KafkaMessagesSentDirectories.Inc()
			}
		}

		// ----------- Отправка количества найденных директорий в топик ------------
	...
	if len(files) != 0 {
		...
		FilesFoundByScanTypes.Add(float64(len(files) * len(scanMsg.ScanTypes)))
		// ------------ Отправка найденных файлов в топик ------------
		...
		// ----------- Отправка количества найденных файлов в топик умноженное на количество типов сканирования ------------
	...
	// ----------- Отправка 1 обработанной директории в топик -----------
