...
var (
	// Вектор и обычная метрика для количества запросов на получение отчёта
	...
	// Вектор и обычная метрика для количества ошибок при получении отчёта
...
func InitMetrics(instance string) {
	...
	reportRequestsTotalVec = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Общее количество запросов на получение отчёта",
		},
		[]string{"instance"},
	)
	reportProcessingDurationVec = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			...
			Help:    "Время обработки запроса получения отчёта (от запроса до ответа)",
	...
	reportErrorsTotalVec = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Количество ошибок при получении отчёта",
		},
		[]string{"instance"},
	)
